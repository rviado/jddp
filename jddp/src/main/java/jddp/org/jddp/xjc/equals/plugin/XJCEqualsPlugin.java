...
public class XJCEqualsPlugin extends AbstractCodeGeneratorPlugin<EqualsArguments> {

	@Override
	public String getOptionName() {
		return "XnotSoSimpleEquals";
	}

	@Override
	public String getUsage() {
		return "  -XnotSoSimpleEquals :  Generate reflection-free runtime-free equals(Object that) methods.\n"
	...
	protected void generate(ClassOutline classOutline, JDefinedClass theClass) {
		...
		if (fields.length > 0) {
			...
			for (final FieldOutline fieldOutline : fields) {
				...
				final FieldAccessorEx leftFieldAccessor = getFieldAccessorFactory().createFieldAccessor(fieldOutline,
						_this);
				final FieldAccessorEx rightFieldAccessor = getFieldAccessorFactory().createFieldAccessor(fieldOutline,
						_that);
				...
				// final JExpression leftHasSetValue = exposedType.isPrimitive()
				// ? JExpr.TRUE
				// : leftValue.ne(JExpr._null());
				final JExpression leftHasSetValue = (leftFieldAccessor.isAlwaysSet()
						|| leftFieldAccessor.hasSetValue() == null) ? JExpr.TRUE : leftFieldAccessor.hasSetValue();

				// final JExpression rightHasSetValue =
				// exposedType.isPrimitive() ? JExpr.TRUE
				// : rightValue.ne(JExpr._null());

				final JExpression rightHasSetValue = (rightFieldAccessor.isAlwaysSet()
						|| rightFieldAccessor.hasSetValue() == null) ? JExpr.TRUE : rightFieldAccessor.hasSetValue();
